...
pub fn SearchContent() -> impl IntoView {
    ...
    view! {<div class="overflow-y-auto max-h-[50vh] space-y-6">
    {search_categories.into_iter().map(|category| {
        let category_str = category.to_string();
        view! {
            <div class="space-y-3">
                <h3 class="text-lg font-semibold text-foreground border-b border-border pb-2">{category_str.clone()}</h3>
                <div class="space-y-2">
                {plant_results.plants.clone().into_iter().map(|plant| {
                                              ...
                                              <div class="flex-1 min-w-0 flex flex-col justify-between">
                                                <div>
                                                  <div class="flex items-center gap-3 mb-2">
                                                    <h4 class="font-medium text-card-foreground">{plant.common_name}</h4>

                                                  </div>
                                                  <p class="text-muted-foreground mb-3 line-clamp-2 text-xs">{plant.description}</p>
                                                </div>
                                                ...
                                                <div class="flex items-center justify-between">
                                                  ...
                                                  <Show when=move || plant.in_garden fallback=move || view! {<Badge
                                                  >
...
pub fn SearchDialog(
    ...
    view! {<Show when=move || !search_input.get().is_empty()>
        <div
        ...
        <div class=move || {
            ...
            <div class="justify-between flex flex-col h-[calc(100%-80px)]">
                ...
                <div tabindex="0" on:keydown=keydown_handler>
                    <FormInputField icon=view! { <Search /> } placeholder="Search" value=search_input on_input=set_search_input/>
                </div>
                    <a href="/">
                        <button class=move || {
